...
! Variables
  Integer(kind=LongInteger) :: randomLCG_n  
...
! Loop through points
  ...
  searchLoopsIn,filterLoopsIn,varyLoopsIn) RESULT (output)
! Fits double exponential to data
! f(x) = a exp(lA) + b exp(lB)
    Implicit None  !Force declaration of all variables
! Declare variables
    Integer(kind=StandardInteger) :: i, j, n, m, r
    ...
    Real(kind=DoubleReal), Dimension(0:4) :: output
    Real(kind=DoubleReal), Dimension(1:100,1:2) :: lRange
    Real(kind=DoubleReal), Dimension(1:100,1:2) :: aRange
    Integer(kind=StandardInteger) :: filterLoops, searchLoops, varyLoops, gridA
    Integer(kind=StandardInteger), Optional :: searchLoopsIn, filterLoopsIn, varyLoopsIn, gridAIn
...
! Optional   
    ...
    End If  
    varyLoops = 10000
    If(Present(varyLoopsIn))Then
      varyLoops = varyLoopsIn
    End If      
! Init
    topBoxCount = 15
    topBoxes = 2.0D20
...
! Start refinement loop
    filterLoops = ceiling(1.0D0*filterLoops/5.0D0)  ! 5 refinement loops
    Do r=1,5
...
! Filter out top boxes      
      Do n=1,topBoxCount
        ...
        lB = topBoxes(n,5)
        Do i=1,filterLoops
          a_T = topBoxes(n,2)*(1+variation*(RandomLCG()-0.5D0))
          lA_T = topBoxes(n,3)*(1+variation*(RandomLCG()-0.5D0))
          b_T = topBoxes(n,4)*(1+variation*(RandomLCG()-0.5D0))
          lB_T = topBoxes(n,5)*(1+variation*(RandomLCG()-0.5D0))
          rss = DoubleDecayFitRSS(dataPoints, a_T, b_T, lA_T, lB_T)
...
! Store results
    ...
    lB = topBoxes(1,5)  
! Vary loops
    varyLoops = ceiling(1.0D0*(varyLoops/50))
! Improve results
    variation = 0.1D0    
    Do i=1,varyLoops
! Update every 50
      Do j=1,50
        a_T = a*(1+variation*(RandomLCG()-0.5D0))
        lA_T = lA*(1+variation*(RandomLCG()-0.5D0))
        b_T = b*(1+variation*(RandomLCG()-0.5D0))
        lB_T = lB*(1+variation*(RandomLCG()-0.5D0))
        rss = DoubleDecayFitRSS(dataPoints, a_T, b_T, lA_T, lB_T)
        If(rss.lt.optRSS)Then
          topBoxes(1,1) = rss
          topBoxes(1,2) = a_T
...
! Update with optimum
      ...
      lB = topBoxes(1,5)
    End Do     
! Adjust each parameter separately
    variation = 0.001D0    
    Do i=1,4
! Update every 50
      Do j=1,1000
        ...
        b_T = b
        lB_T = lB
        If(i.eq.1)Then !Vary lA
          lA_T = lA*(1+variation*(RandomLCG()-0.5D0))
        End If  
        If(i.eq.2)Then !Vary lB
          lB_T = lB*(1+variation*(RandomLCG()-0.5D0))
        End If  
        If(i.eq.3)Then !Vary a
          a_T = a*(1+variation*(RandomLCG()-0.5D0))
        End If  
        If(i.eq.4)Then !Vary b
          b_T = b*(1+variation*(RandomLCG()-0.5D0))
        End If  
        rss = DoubleDecayFitRSS(dataPoints, a_T, b_T, lA_T, lB_T)
        If(rss.lt.optRSS)Then
